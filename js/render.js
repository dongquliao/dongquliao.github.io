...
// 手机端显示详情弹窗（包含歌词）
function showMobileDetail(song) {
    if (!song) return;
    ...
    if (!modal) {
        ...
        modal.innerHTML = `
            <div class="mobile-detail-content">
                ...
                <div class="mobile-detail-lyrics" id="mobileDetailLyrics">
                    <div class="lyrics-label">歌词：</div>
                    <div class="lyrics-content" id="mobileLyricsContent"></div>
                </div>
            </div>
        `;
    ...
    // 填充歌词
    const lyricsContent = document.getElementById('mobileLyricsContent');
    if (song.lyrics && song.lyrics.trim()) {
        lyricsContent.innerHTML = `<pre style="white-space: pre-wrap; font-family: inherit; font-size: 13px; line-height: 1.6; margin: 0;">${escapeHtml(song.lyrics)}</pre>`;
    } else {
        lyricsContent.innerHTML = '<div class="no-lyrics" style="color: #999; font-style: italic; padding: 10px 0;">暂无歌词</div>';
    }
    
    // 显示弹窗
    modal.style.display = 'flex';
    
    // 关键修复：延迟重置滚动位置，确保 DOM 完全渲染后再执行
    setTimeout(() => {
...
export function renderTable(onSelectSong, onShowLyrics) {
    ...
    displaySongs.forEach((song, index) => {
        ...
        if (isMobile) {
            // 手机端：单击显示详情弹窗（包含歌词）
            row.onclick = () => showMobileDetail(song);
            // 保留双击打开独立歌词弹窗（备用）
            row.ondblclick = (e) => {
                e.stopPropagation();
                onShowLyrics(song);
